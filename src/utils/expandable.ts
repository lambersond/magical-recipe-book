...
export const updateStateAttributeAdvanced =
  (id: string) => (e: MouseEvent<HTMLButtonElement>) => {
    ...
    for (const section of expandableSections) {
      if (section.id === id) section.dataset.state = newState
    }
  }
...
export const expandablePaneClasses =
  'overflow-hidden transition-all duration-300 ease-in-out data-[state=false]:max-h-0 data-[state=false]:opacity-0 data-[state=false]:mt-0 data-[state=true]:max-h-[1000px] data-[state=true]:opacity-100 data-[state=true]:mt-2'
